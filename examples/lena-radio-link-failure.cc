...
NS_LOG_COMPONENT_DEFINE("LenaRadioLinkFailureExample");
...
void PhySyncDetection(uint64_t imsi, uint16_t rnti, uint16_t cellId, std::string type, uint16_t count)
{
std::cout << Simulator::Now().GetSeconds()
                        << " IMSI " << imsi <<", RNTI " << rnti
                        << ", CellId " << cellId << ", "<<type<< ", no of sync indications: "<<count
                        << std::endl;
}

void RadioLinkFailure (uint64_t imsi, uint16_t rnti, uint16_t cellId)
{
  std::cout << Simulator::Now().GetSeconds()
                        << " IMSI " << imsi <<", RNTI " << rnti
                        << ", CellId " << cellId <<", radio link failure detected "
                        << std::endl<<std::endl;
}
 ...
 * Only one eNodeB and one UE is considered for verifying
...
//  LogComponentEnable ("LteEnbRrc", logLevel);
  LogComponentEnable ("LenaRadioLinkFailureExample", logLevel);
  ...
  //Set the TimeAlignmentTimer to max value so that only radio link failure can be simulated
